...
// ─────────── 本地补丁（xray-wasm · V24 诊断累加器）───────────
// 由 `block_on` 每 0.5s `diag_take()` 取走（读清），用于在**空转期间**观测
// `check_pollables` 的调用速率、目标数与就绪数。
static DIAG_CALLS: AtomicU64 = AtomicU64::new(0);
static DIAG_TARGETS: AtomicU64 = AtomicU64::new(0);
static DIAG_READY: AtomicU64 = AtomicU64::new(0);
...
impl Reactor {
    ...
    /// 本地补丁（xray-wasm · V24 诊断）：读清三个累加器，返回本区间增量。
    pub(crate) fn diag_take(&self) -> (u64, u64, u64) {
        (
            DIAG_CALLS.swap(0, Ordering::Relaxed),
            DIAG_TARGETS.swap(0, Ordering::Relaxed),
            DIAG_READY.swap(0, Ordering::Relaxed),
        )
    }
        ...
        F: FnOnce(&[&Pollable]) -> Vec<u32>,
    {
        ...
        let mut indexed_waitees = Vec::with_capacity(wakers.len());
        let mut targets = Vec::with_capacity(wakers.len());
        for waitee in wakers.keys() {
            let pollable_index = waitee.pollable.0.key;
            indexed_waitees.push(waitee.clone());
        ...
        DIAG_CALLS.fetch_add(1, Ordering::Relaxed);
        DIAG_TARGETS.fetch_add(targets.len() as u64, Ordering::Relaxed);
        let ready_indexes = check_ready(&targets);
        DIAG_READY.fetch_add(ready_indexes.len() as u64, Ordering::Relaxed);
        drop(targets);
        drop(pollables);

        // ─────────── 本地补丁（xray-wasm · V24）· 唤醒即摘除 ───────────
        // 上游只在 `WaitFor::drop`（且 `needs_deregistration`）时注销 waitee。
        // 孤儿 waitee + 永久就绪 pollable ⇒ `block_on` 循环空转、其它任务全被饿死。
        // 实测（2026-09-21）：**只靠这一处不够**（hover300 marginal 0.7045→0.3858），
        // 与 `xt-wasm-runtime::Ready::futile` 兜底**合起来**才把三判据全部压过
        // （hover300 marginal 0.0179 / curl 200 / 回落 0.0054，裁定「不复现」）；
        // 反之只留兜底、撤掉这里时回落 marginal 又回到 1.8458。两处都需要。
