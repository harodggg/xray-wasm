...
// (crates/meow-proxy/src/vless/header.rs).
// MIT licensed, Copyright (c) 2026 Max Lv. See LICENSE.meow-rs.
//
// Modified for the wasm32-wasip2 port: `smol_str::SmolStr` → `String`; the
// Mux.Cool request encoder (behind upstream's `mux` feature) is out of scope and
// dropped; `pub(crate)` items are `pub` so the crate's callers can reach them.
...
pub enum VlessAddr {
    ...
    ///
    /// Upstream used `SmolStr`; the port uses `String` (port-map §8.5) so the
    /// `smol_str` dependency — and its rustc-1.89 MSRV — is not needed.
    Domain(String),
}
...
impl VlessAddr {
    ...
    pub fn domain(s: &str) -> std::result::Result<Self, String> {
        if s.len() > 255 {
            return Err(format!(
...
pub fn addr_from_metadata(m: &Metadata) -> VlessAddr {
    if !m.host.is_empty() {
...
pub enum Cmd {
    Tcp = 0x01,
    Udp = 0x02,
    // NOTE: upstream's `Mux = 0x03` variant (behind the `mux` feature) is out of
    // scope for the TCP/Vision port and is intentionally absent.
}
...
// by `VlessConn::poll_read` (TCP) with persistent, cancellation-safe progress
// in `vless/conn.rs`.
...
mod tests {
    ...
    // Ported from upstream `#[tokio::test]`: no tokio runtime is needed for an
    // in-memory `duplex`, so the body runs under `xt_wasm_runtime::block_on`.
    #[test]
    fn header_encode_full_round_trip_via_decode() {
        use tokio::io::duplex;
    ...
    // Response-header decode coverage (B1-B4) lives with the consumer
    // now: `vless/conn.rs` tests the response reader for version mismatch,
