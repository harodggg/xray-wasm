...
### V27 终局（2026-09-21）：那条「最小验证实验」做完了，假设**被证伪**

板子上面留的那条实验（「让客户端把首条应用记录与 Finished 错开，看 5s 是否消失」）已跑：

| 客户端错开 | ServerHello | **VLESS-at-server**（服务端**处理**首条记录） | 首连 TTFB |
|---|---|---|---|
| 0ms（基线） | +0.40…0.46 | +5.40…+5.46 | 5.76–6.09s |
| **30ms** | +0.25…1.19 | **+5.25…+5.27** | 5.56–6.48s |
| **1000ms** | +0.25 | **+5.25** | 5.55–5.63s |

`+VLESS-at-server − +ServerHello ≈ 5.00s` **与客户端何时发首条应用记录无关** ⇒
不是「同突发/丢通知」，是官方服务端的**固定计时器**（一次性初始化）。

又实现了启动预热（bind 前 `spawn_task` 一次性隧道，**等到第一个响应字节**才算数）：
预热本身确实吸收（日志 `预热完成：5.50s`）、`v27-cold-warm.sh e2e` **3/3 PASS**，
**但初始化窗口内到达的用户请求仍要等 ~5.2s**（TTFB 5.54–5.59s）⇒ 预热并不能让
「启动后立刻发的第一条请求」变快，只有**初始化完成之后**发的连接才是 ~0.2–0.3s。

⇒ **结论：V27 的这 5s 客户端消不掉，只能「挪走」**。已交付（v0.7.2）：
① 已发布的握手重试（覆盖「首连超时失败」）；② **`--warmup-gate` / `XT_WARMUP_GATE=1`**
（预热完成后才 bind，默认关闭）——用户首条请求 TTFB **5.76–6.09s → 0.95–1.14s**，
`v27-cold-warm.sh e2e` 开门 3/3、关门回归通过；`e2e-test.sh` 的就绪等待改为轮询
（`XT_TEST_CLIENT_WAIT_SECS`，默认 20s），启动日志移到 bind 之后打印。
完整数据与建议见 `docs/findings/v27-not-client-fixable.md`。
