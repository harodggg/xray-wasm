...
### V24 十九续：**写路径那个「修复」是假的 —— 它把忙等换成了阻塞，我误读了探针**

V24 十六/十七续我报告「`blocking_write_and_flush` 修复有效，两个探针 501 → 0」。
**那个结论是错的。**
...
==> 2/5 正向：--no-flow 的 wasm 客户端 → wasm 服务端
  ✗ wasm→wasm 正向不通（拿到 000）
```

回滚之后同一测试全绿。所以那个改动**破坏了正常功能**。

#### 我为什么会误判

探针的判据是「CPU ticks 接近 0」。而 `blocking_write_and_flush` 是**阻塞**调用 ——
在单线程运行时里它会把**整个实例**卡住。**卡住时 CPU 同样是 0。**

> ⇒ **「CPU = 0」这个判据无法区分「修好了」和「全卡住了」。**

这是一个**判据设计错误**，不是测量错误。我用一个单侧判据（CPU 低）去判定一个双侧问题
（正常 = CPU 低且功能正常；阻塞 = CPU 也是低），于是把「整个实例死锁」读成了「修好了」。

**正确判据必须是双侧的**：CPU 低 **且** 功能仍然通过（例如同一个探针里再发一条
正常请求并断言成功响应）。这正是既有 e2e 的价值 —— 它抓住了探针抓不到的东西。

#### 处置

* 已回滚该改动（`git checkout 2a3c0b9 -- wasi.rs`），功能 e2e 复验全绿；
* **探针判据必须补上功能断言**，否则它以后还会骗人；
* 结论回到 V24 十八续：**第二个源仍在，且未修复**。
  `check_write()` 恒为 0 是**真实的观测**，但「改用阻塞写」不是它的解法。

#### 教训（这一条比 bug 本身重要）

**只测「症状消失」是不够的，必须同时测「功能还在」。** 我在同一件事上已经栽过：
前面用「只读探针 0 ticks」推断「socket 层没问题」，也是单侧判据。
**每一次用单侧指标下结论，都出了错。**

### V24 二十续：握手超时**无效** ⇒ 自旋在握手**之后**

按「先限制爆炸半径、再追根因」的思路，给 REALITY 握手加了 30 秒时间预算
（`HANDSHAKE_TIMEOUT`），理由是实测的自旋是「每次 poll 返回 Pending、18,336 次/秒」，
所以 `futures::future::select` 有机会轮询到定时器并触发。

实测（**把取样点推到超时之后**，因为第一次测的 17 秒窗口比 30 秒超时还短，
那次证明不了任何事）：
...
**自旋不在握手阶段。** 结合此前「这批连接死在 `decode_request`（VLESS 请求头 early eof）」
的日志，范围缩到**握手之后**：

1. `decode_request` 的读（等 VLESS 请求头）
2. 或 `connect()`（目标不可达时挂住）

这两个的先后是：先 `decode_request` 成功（读到请求头），再 `connect`。
而日志显示连接**死在 decode_request**（early eof）——所以**嫌疑最大的是
`decode_request` 的读路径**，而它恰恰是我们**还没在「已写过」的组合下单独测过**的那一格。

#### 下一刀（明确）

在 socket 层探针上做**「写 + 读，且读到的数据不足以填满请求」**：写 3 轮 →
读到一个短消息 → **再读一次并永久挂住**。这正好对应「握手写过 → 解码请求头读到一半
→ 等剩余字节」的形状。判据：0 = 不是它；~500 = 就是它。

#### 现状

* 交易：**未修复**，爆炸半径未限制住；
* 线上仍靠 `livenessProbe`（实测 2.7 小时内捕获 1 次并自愈，`restartCount=1`）；
* 已交付的是测试、探针、和一份记录了 20 轮（含 4 次撤回）的 V24。

### V24 二十一续：**进程内的超时救不了 —— 自旋把定时器也饿死了**

给 `decode_request` 加了 10 秒预算（`REQUEST_TIMEOUT`），用**干净端口**重跑，
取样点跨过超时（+8 / +25 / +50 / +80 秒）：

```
+8s: 501 ticks     +25s: 501 ticks     +50s: 500 ticks     +80s: 501 ticks
日志行数 = 3（只有 banner）    outcome=Rejected 条数 = 0
```

**超时没有触发，一条拒绝日志都没有。** 300 条连接全部没有产生任何 outcome。

#### 结论：**进程内的任何超时都不可能生效**

原因是自旋的性质：它不只是一个占用 CPU 的忙循环，**它还让 reactor 无法安静下来去
处理定时器事件**（`wstd::time::Timer` 依赖 `poll_oneoff` 的正常调度）。
定时器被一起饿死了，所以 `futures::future::select(任务, 定时器)` 里的定时器分支
永远不会赢。

这**一次性解释**了前面两次失败的尝试：

* V24 二十续：握手加 30 秒预算 → 无效（我当时以为是"阶段不对"）；
* 本次：请求头加 10 秒预算 → 无效（连日志都没有）。

**不是阶段选错了，是这条机制在自旋面前根本不可用。**

#### 那什么才有用：**进程外**的监督

| 手段 | 能否生效 | 原因 |
|---|---|---|
| 进程内超时（握手 / 请求头 / 连接级） | ❌ | 定时器被自旋饿死 |
| `tokio::time`（不可用，无 reactor） | ❌ | — |
| **`livenessProbe`（kubelet 探活后重启）** | ✅ | **监督者在进程外**，不受自旋影响 |

这也解释了为什么线上那个 `livenessProbe` **实测确实有效**（2.7 小时捕获 1 次，
`restartCount=1`），而我在进程内折腾了 3 轮全失败。

> **教训：当一个故障会让事件循环本身失效时，"在里面加超时"是结构性无效的，
> 只能从外面兜。** 这一点应该在动手前就想清楚 —— 一个连 accept 循环都能饿死的
> 忙等，凭什么会留出时间给定时器？
...
## V30 · V27 终局：5s 是官方服务端的固定计时器，客户端以「就绪门」把它挪到监听之前

### 假设证伪

客户端把首条应用记录相对 Finished 错开 **30ms / 1000ms**（研究开关 `XT_FIRST_RECORD_DELAY_MS`），
官方服务端仍在 ServerHello 之后**恒定 +5.00s** 处理该记录（+VLESS-at-server − +ServerHello ≈ 5.00s，
与错开量无关）⇒ 「同突发 / 丢通知」假设**证伪**；是服务端侧的固定计时器（一次性初始化）。

### 窗口效应（关键）

异步预热（不等就绪门）确实吸收了自己那 5s（日志 `预热完成：5.50s`、`v27-cold-warm.sh e2e` 3/3 PASS），
**但初始化窗口内到达的用户请求仍要等 ~5.2s**（TTFB 5.54–5.59s）。
⇒ 只有**把监听推迟到预热之后**才能让用户首条请求变快。

### 交付（v0.7.2，opt-in）

`--warmup-gate` / `XT_WARMUP_GATE=1`：先建一条一次性隧道并**等到第一个响应字节**，再 bind。

| 指标 | 基线（默认） | 就绪门 |
|---|---|---|
| 用户首条请求 TTFB（官方服务端冷启动，3 次） | 5.76–6.09s | **0.95 / 1.06 / 1.14s** |
| `v27-cold-warm.sh e2e` | 0/3（固定 `sleep 2` 等不住） | **3/3 PASS**（15.7–20.2s） |

配套：`e2e-test.sh` 就绪等待改轮询（`XT_TEST_CLIENT_WAIT_SECS`，默认 20s）；
「监听 / 用法」日志移到 bind 成功之后；门提示语不含「监听」二字（避免被 `grep 监听` 误判就绪）。
门默认关闭，不改变既有启动契约。证据链：`docs/findings/v27-not-client-fixable.md`。
